...
proj_den = np.sqrt( d3.Integrate(opt.get_metric('Rsqrd').quantity) * d3.Integrate(opt.get_metric('Rsqrd').quantity) )
...
opt.objective_overwrite = 'default'
quan = opt.get_metric(objective_overwrite)
if quan == None:
    try:
        quan = eval(objective_overwrite)
        if isinstance(quan, FutureField):
            opt.objective_overwrite = quan
    except:
...
if (load_state):
    try:
        ...
        opt.ic['u'][opt_layout] = opt.reshape_soln(loadu)
    except:
...
try:
    ...
    # print('rank = {}; shape x0 = {}'.format(CW.rank, np.shape(x0)))
    # sys.exit()
