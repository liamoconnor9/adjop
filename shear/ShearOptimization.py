...
class ShearOptimization(OptimizationContext):
    ...
    def before_fullforward_solve(self):
        if self.add_handlers and self.loop_index % self.handler_loop_cadence == 0:
            checkpoints = self.forward_solver.evaluator.add_file_handler(self.run_dir + '/' + self.write_suffix + '/checkpoints/checkpoint_loop'  + str(self.loop_index), max_writes=1, sim_dt=self.T - self.dt, mode='overwrite')
    ...
    def during_fullforward_solve(self):
        # if (self.forward_solver.iteration % 200 == 1):
        #     logger.info('Forward solver; iteration = {}; time = {}'.format(self.forward_solver.iteration, self.forward_solver.sim_time))
        pass
    ...
    def after_fullforward_solve(self):
        ...
        if (CW.rank == 0  or self.domain.dist.comm == MPI.COMM_SELF):
            with open(self.run_dir + '/' + self.write_suffix + '/output.txt', 'a') as f:
                f.write(loop_message)
                f.write('\n')
    ...
    def during_backward_solve(self):
        # if (self.backward_solver.iteration % 200 == 1):
        #     logger.info('Backward solver; iteration = {}; time = {}'.format(self.backward_solver.iteration, self.backward_solver.sim_time))
