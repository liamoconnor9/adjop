...
def test_solves2():
    ...
    opt.set_time_domain(T, num_cp, dt*10)

    f = opt.loop(U0g.copy() + 0.1*np.sin(x))
    ...
    Failed = f > 1e-2
    Failed *= np.max(fprime**2) > 1e-2

    # verify that a trivial guess gives trivial gradient and objective
    ...
    # verify that solves are reproducible
    f = opt.loop(U0g.copy() + 0.1*np.sin(x))
    fprime = opt.jac(opt.x)
    logger.info(f)
    logger.info(np.max(fprime**2))
    ...
    Failed *= (f - f_og) == 0.0
    Failed *= np.max((fprime - fprime_og)**2) == 0.0
