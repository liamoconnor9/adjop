...
try:
    ...
    default_suffix = 'test'
    filename = "{}/{}/{}.cfg".format(path, default_suffix, default_suffix)
...
class OptimizationSerial(*Features):
    def loop_message(self):
        # print('called by' + sys._getframe().f_back.f_code.co_name)
        ...
        for metname in self.tracker.keys():
            if metname == 'x_lst' or metname == 'x':
                # don't print this, it's an array (but its in the tracker cuz 1D)
                continue
...
opt.add_metric('Rsqrd', True, 1, 0.5*(opt.ic['u'] - ubar)**2, integrate=True)

opt.add_metric('x', True, 1, opt.ic['u'], integrate=False)
...
opt.add_metric('obj_approx', False, 1, 0.5*(u_t * u_t), integrate=True)

proj_num = d3.Integrate(0.5*(opt.ic['u'] - ubar) * u_t)
...
opt.soln = soln
# opt.objective_overwrite = opt.get_metric('obj_approx').quantity
...
try:
    ...
    x0 = opt.ic['u'][opt.opt_layout].flatten().copy()  # Initial guess.
